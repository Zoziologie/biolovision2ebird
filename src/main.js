  ...
  methods: {
    ...
    protocol(f) {
      const protocol = {
        invalid: {
          ...
          variant: "danger",
          title:
            "Your checklist is currently not acceptable for eBird. You are probably missing a Date.",
        },
        incidental: {
          name: "Incidental",
          letter: "I",
          variant: "warning",
          title:
            "Incidental protocol is used when birding was not you primary purpose. The checklist will have little value.",
        },
        historical: {
          name: "Historical",
          letter: "H",
          variant: "warning",
          title:
            "Can you add all effort information (time, duration, party size and distance)? This would really improve the value of your data!",
        },
        stationary: {
          name: "Stationary",
          letter: "S",
      ...
      if (form.static_map.show) {
        ...
        if (form.static_map.gist) {
          comment +=
            ...
            encodeURIComponent(form.static_map.gist) +
            "'>" +
            img +
            "</a>";
        } else {
    ...
    staticMapLink(form, sightings) {
      ...
      if (form.static_map.bounding_box_auto) {
        ...
        let s = bounds.pad(0.1).getSouth();
        let n = bounds.pad(0.3).getNorth();
        let e = bounds.pad(0.2).getEast();
        let w = bounds.pad(0.2).getWest();
        if (n - s < dlat) {
          let c = (n + s) / 2;
          n = c + dlat / 2;
          s = c - dlat / 2;
      ...
      return `https://api.mapbox.com/styles/v1/mapbox/${form.static_map.style}/static/${path}${markers}/[${form.static_map.bounding_box}]/300x200?access_token=${form.static_map.mapbox_token}&logo=false`;
    },
